...
FIGURE_ID = "ch12_05_peer_review_form"


def main() -> None:
    """
    RU:
        Подраздел 12.5. Тезис: форма экспертной проверки помогает отделить
        вкусовые замечания от методических ошибок.

    EN:
        Subsection 12.5. Thesis: a peer-review form helps separate taste-based
        comments from methodological errors.
    """
    form = pd.DataFrame({
        "question": [
            "Понятен ли главный вывод рисунка?",
            "Соответствует ли тип графика типу данных?",
            "Есть ли единицы измерения на осях?",
            "Не скрыта ли вариабельность?",
            "Корректно ли объяснены интервалы/ошибки?",
            "Читабелен ли рисунок в чёрно-белом виде?",
            "Достаточно ли качества экспорта?",
        ],
        "answer": ["да/нет/частично"] * 7,
        "comment": [""] * 7,
    })
    save_table(form, "ch12_05_peer_review_form.csv")
