...
Run:
    uv run python chapters/chapter_09_composite_export/ch09_03_common_colorbar.py
...
FIGURE_ID = "ch09_03_common_colorbar"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 9.3. Тезис: при нескольких панелях с одной непрерывной
        цветовой переменной лучше использовать общую цветовую шкалу.

    EN:
        Subsection 9.3. Thesis: for several panels sharing one continuous color
        variable, a common colorbar is preferable.
    """
    ...
    axes[1].set_title("B. Глюкоза и АД")
    axes[1].set_xlabel("Глюкоза, ммоль/л")
    axes[1].set_ylabel("Систолическое АД, мм рт. ст.")
    add_panel_label(axes[1], "B")
    sns.despine(ax=axes[1])

    cbar = fig.colorbar(scatter, ax=axes, shrink=0.85, pad=0.03)
    cbar.set_label("Сконструированный риск")

    fig.suptitle("Общая цветовая шкала для двух панелей", y=1.03)
    # fig.tight_layout()
    return fig
