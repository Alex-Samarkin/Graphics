...
Run:
    uv run python chapters/chapter_09_composite_export/ch09_01_panel_labels_consistency.py
...
FIGURE_ID = "ch09_01_panel_labels_consistency"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 9.1. Тезис: буквенные метки панелей должны быть
        одинаково расположены и оформлены во всём рисунке.

    EN:
        Subsection 9.1. Thesis: panel letters should be consistently placed
        and formatted throughout the figure.
    """
    fig, axes = plt.subplots(1, 3, figsize=(12, 4), sharey=True)

    variables = [
        ("age_years", "Возраст, годы", "A"),
        ("bmi_kg_m2", "ИМТ, кг/м²", "B"),
        ("glucose_mmol_l", "Глюкоза, ммоль/л", "C"),
    ]

    for ax, (x, xlabel, label) in zip(axes, variables):
        sns.scatterplot(data=df, x=x, y="sbp_mm_hg", hue="therapy_group", alpha=0.65, ax=ax, legend=False)
    ...
    fig.suptitle("Единые метки панелей A–C", y=1.04)
    fig.tight_layout()
    return fig
