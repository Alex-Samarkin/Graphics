...
Run:
    uv run python chapters/chapter_08_axes_legends_captions/ch08_04_caption_self_contained.py
...
FIGURE_ID = "ch08_04_caption_self_contained"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 8.4. Тезис: подпись к рисунку должна быть самодостаточной:
        что показано, кто сравнивается, что означают линии/точки/интервалы.

    EN:
        Subsection 8.4. Thesis: a figure caption should be self-contained:
        what is shown, what is compared, and what lines/points/intervals mean.
    """
    ...
    ax.set_title("Среднее АД и стандартное отклонение")

    caption = (
        f"Рисунок. Среднее систолическое АД в группах терапии "
        f"(общий объём выборки n = {len(df)}). Точки показывают среднее, "
        "вертикальные линии — стандартное отклонение (SD), а не SEM и не 95% CI."
    )
