...
FIGURE_ID = "ch08_01_axis_labels_units_bad_good"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 8.1. Тезис: оси должны иметь понятные названия и единицы.
        Подписи вроде x/y/value неприемлемы для научной работы.

    EN:
        Subsection 8.1. Thesis: axes should have clear names and units.
        Labels such as x/y/value are unacceptable in scientific figures.
    """
    fig, axes = plt.subplots(1, 2, figsize=(10, 4), sharey=True)

    ax = axes[0]
    ax.scatter(df["bmi_kg_m2"], df["sbp_mm_hg"], alpha=0.6, s=24)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Плохо: нет смысла и единиц")
    sns.despine(ax=ax)
    ...
    ax.set_title("Лучше: переменные и единицы")
    sns.despine(ax=ax)
