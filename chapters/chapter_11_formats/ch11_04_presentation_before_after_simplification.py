...
Run:
    uv run python chapters/chapter_11_formats/ch11_04_presentation_before_after_simplification.py
...
FIGURE_ID = "ch11_04_presentation_before_after_simplification"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 11.4. Тезис: график для слайда должен быть проще, чем
        график для статьи; второстепенные детали лучше убрать.

    EN:
        Subsection 11.4. Thesis: a slide figure should be simpler than
        an article figure; secondary details should be removed.
    """
    summary = df.groupby(["week", "therapy_group"], as_index=False).agg(
    ...
    ax.set_title("Слишком подробно для слайда")
    ax.set_xlabel("Неделя")
    ax.set_ylabel("Среднее АД")
    ax.legend(title="Группа")
    sns.despine(ax=ax)

    ax = axes[1]
    active = summary[summary["therapy_group"].isin(["Препарат A", "Плацебо"])]
    sns.lineplot(data=active, x="week", y="mean_sbp", hue="therapy_group", marker="o", linewidth=3, ax=ax)
    ax.set_title("Слайд: только главный контраст")
    ax.set_xlabel("Неделя")
    ax.set_ylabel("Среднее АД")
    ax.legend(title="")
    sns.despine(ax=ax)
