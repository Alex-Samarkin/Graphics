...
Run:
    uv run python chapters/chapter_11_formats/ch11_07_responsive_square_card.py
...
FIGURE_ID = "ch11_07_responsive_square_card"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 11.7. Тезис: для карточек сайта или соцсетей удобен
        квадратный формат с крупным главным сообщением.

    EN:
        Subsection 11.7. Thesis: website/social cards benefit from a square
        format with a large key message.
    """
    summary = df.groupby("therapy_group", as_index=False).agg(mean_sbp=("sbp_mm_hg", "mean"))

    fig, ax = plt.subplots(figsize=(6, 6))
    sns.barplot(data=summary, x="therapy_group", y="mean_sbp", edgecolor="black", ax=ax)

    ax.set_xlabel("")
    ax.set_ylabel("Среднее АД, мм рт. ст.")
    ax.set_title("Сравнение групп терапии")
    ...
    fig.text(
        0.5,
        0.02,
        "Square card: меньше деталей, крупнее текст, один вывод",
        ha="center",
        va="bottom",
        fontsize=10,
    )
