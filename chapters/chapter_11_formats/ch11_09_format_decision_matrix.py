...
Run:
    uv run python chapters/chapter_11_formats/ch11_09_format_decision_matrix.py
...
FIGURE_ID = "ch11_09_format_decision_matrix"


def build_figure() -> plt.Figure:
    """
    RU:
        Подраздел 11.9. Тезис: выбор формата зависит от ситуации:
        статья, ВКР, презентация, постер, веб или печать.

    EN:
        Subsection 11.9. Thesis: format choice depends on context:
        article, thesis, presentation, poster, web, or print.
    """
    matrix = pd.DataFrame({
        "Контекст": ["Статья", "ВКР/диссертация", "Презентация", "Постер", "Веб", "Печать"],
        "Размер": ["1–2 колонки", "средний/крупный", "16:9", "крупный", "адаптивный", "по требованиям"],
        "Шрифт": ["малый, но читаемый", "умеренный", "крупный", "очень крупный", "средний", "читаемый"],
        "Формат": ["PDF/TIFF/SVG", "PNG/PDF", "PNG/PDF", "PDF/PNG", "PNG/SVG/HTML", "PDF/TIFF"],
    })
    ...
    table.scale(1, 1.45)
    ax.set_title("Матрица выбора формата научного рисунка", pad=18)
    fig.tight_layout()
    return fig
