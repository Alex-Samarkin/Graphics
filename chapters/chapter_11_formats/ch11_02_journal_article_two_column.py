...
Run:
    uv run python chapters/chapter_11_formats/ch11_02_journal_article_two_column.py
...
FIGURE_ID = "ch11_02_journal_article_two_column"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 11.2. Тезис: двухколоночный формат статьи позволяет
        использовать несколько панелей, но требует строгой экономии места.

    EN:
        Subsection 11.2. Thesis: a two-column journal figure allows several
        panels but requires strict spatial economy.
    """
    fig, axes = plt.subplots(1, 3, figsize=(7.2, 2.7), sharey=True)

    specs = [
        ("age_years", "Age, years", "A"),
        ("bmi_kg_m2", "BMI, kg/m²", "B"),
        ("glucose_mmol_l", "Glucose, mmol/L", "C"),
    ]

    for ax, (x, xlabel, label) in zip(axes, specs):
        sns.scatterplot(data=df, x=x, y="sbp_mm_hg", alpha=0.45, s=14, ax=ax)
