...
Run:
    uv run python chapters/chapter_11_formats/ch11_06_management_report_horizontal_bars.py
...
FIGURE_ID = "ch11_06_management_report_horizontal_bars"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 11.6. Тезис: для отчётов часто лучше горизонтальные
        столбцы — они читаются быстрее при длинных подписях категорий.

    EN:
        Subsection 11.6. Thesis: reports often benefit from horizontal bars,
        especially when category labels are long.
    """
    summary = (
        df.groupby("department", as_index=False)
        .agg(total_n=("n", "sum"))
        .sort_values("total_n", ascending=True)
    )

    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.barh(summary["department"], summary["total_n"], edgecolor="black")

    ax.set_xlabel("Число пациентов, n")
    ax.set_ylabel("Отделение")
    ax.set_title("Горизонтальная диаграмма для отчёта")
    sns.despine(ax=ax)
