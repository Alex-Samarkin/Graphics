...
Run:
    uv run python chapters/chapter_02_environment/ch02_03_object_oriented_matplotlib.py
...
FIGURE_ID = "ch02_03_object_oriented_matplotlib"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Тезис: объектно-ориентированный стиль matplotlib (`fig, ax`)
        надёжнее, чем неявные команды `plt.plot`, особенно для нескольких панелей.

    EN:
        Thesis: the object-oriented matplotlib style (`fig, ax`) is safer
        than implicit `plt.plot` commands, especially for multi-panel figures.
    """
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    ax = axes[0]
    ax.scatter(df["age_years"], df["sbp_mm_hg"], alpha=0.65, s=22)
    ax.set_xlabel("Возраст, годы")
    ax.set_ylabel("Систолическое АД, мм рт. ст.")
    ax.set_title("A. Возраст и АД")
    sns.despine(ax=ax)

    ax = axes[1]
    ax.scatter(df["bmi_kg_m2"], df["glucose_mmol_l"], alpha=0.65, s=22)
    ax.set_xlabel("ИМТ, кг/м²")
    ax.set_ylabel("Глюкоза, ммоль/л")
    ax.set_title("B. ИМТ и глюкоза")
    sns.despine(ax=ax)
