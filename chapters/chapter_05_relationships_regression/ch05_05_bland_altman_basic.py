...
Run:
    uv run python chapters/chapter_05_relationships_regression/ch05_05_bland_altman_basic.py
...
FIGURE_ID = "ch05_05_bland_altman_basic"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 5.5. Тезис: график Бланда–Альтмана показывает согласие
        двух методов измерения через среднее измерение и разность методов.

    EN:
        Subsection 5.5. Thesis: a Bland-Altman plot shows agreement between
        two measurement methods via mean measurement and method difference.
    """
    import numpy as np
    ...
    ax.axhline(loa_low, linestyle="--", label=f"LoA lower = {loa_low:.1f}")
    ax.axhline(loa_high, linestyle="--", label=f"LoA upper = {loa_high:.1f}")

    ax.set_xlabel("Среднее двух измерений, мм рт. ст.")
    ax.set_ylabel("Автоматический − ручной метод, мм рт. ст.")
    ax.set_title("График Бланда–Альтмана")
    ax.legend()
    sns.despine(ax=ax)
