...
FIGURE_ID = "ch05_03_regression_line_ci"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 5.3. Тезис: линия регрессии показывает ожидаемое среднее
        значение, а доверительная полоса — неопределённость этой оценки.

    EN:
        Subsection 5.3. Thesis: the regression line shows the expected mean
        value, while the confidence band shows uncertainty in that estimate.
    """
    ...
    ax.set_title("Линейная регрессия с 95% доверительной полосой")
    sns.despine(ax=ax)
