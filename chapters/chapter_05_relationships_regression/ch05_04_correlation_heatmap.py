...
Run:
    uv run python chapters/chapter_05_relationships_regression/ch05_04_correlation_heatmap.py
...
FIGURE_ID = "ch05_04_correlation_heatmap"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 5.4. Тезис: тепловая карта корреляций компактно показывает
        структуру взаимосвязей, но не заменяет просмотр scatter plots.

    EN:
        Subsection 5.4. Thesis: a correlation heatmap compactly shows
        association structure but does not replace scatter plots.
    """
    corr = df.drop(columns=["patient_id"]).corr(method="pearson")
