...
Run:
    uv run python chapters/chapter_04_category_comparison/ch04_03_stacked_bar_chart.py
...
FIGURE_ID = "ch04_03_stacked_bar_chart"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 4.3. Тезис: составные столбцы показывают общий объём
        и внутренний состав категорий, но хуже подходят для точного сравнения
        средних сегментов.

    EN:
        Subsection 4.3. Thesis: stacked bars show total volume and internal
        composition, but are weaker for precise comparison of middle segments.
    """
    pivot = df.pivot(index="department", columns="severity", values="n")
    ...
    ax.legend(title="Тяжесть состояния")
