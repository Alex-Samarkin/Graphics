...
Run:
    uv run python chapters/chapter_06_color_palettes/ch06_06_color_redundancy_not_overload.py
...
FIGURE_ID = "ch06_06_color_redundancy_not_overload"


def build_figure(df: pd.DataFrame) -> plt.Figure:
    """
    RU:
        Подраздел 6.6. Тезис: цвет должен помогать чтению, а не создавать
        лишнюю визуальную нагрузку. Не каждую переменную нужно кодировать цветом.

    EN:
        Subsection 6.6. Thesis: color should support reading, not create
        unnecessary visual load. Not every variable should be color-coded.
    """
    ...
    ax.set_title("Перегрузка: цвет + форма + размер")
    ...
    ax.set_title("Чище: цвет только для группы")
    ax.set_xlabel("ИМТ, кг/м²")
    ax.set_ylabel("Систолическое АД, мм рт. ст.")
